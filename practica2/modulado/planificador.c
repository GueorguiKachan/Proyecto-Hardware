			...
			if(evento_id == alarmaSet){														// Programar alarma nueva
				...
				if(pulsacion1_recien_pulsado){
					if(!power_down_active){
						...
						gestorIO_activar_validacion(); // Tras 1 segundo, apagar bit de validación
					}
					else{ 
						power_down_active = false;
						gestorIO_escribir_idle(0);// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!Esto no sobra?
					}
					//cola_guardar_eventos(alarmaSet,eventoPD); !!!!!!!!!!!!!!!!!Añadir un cola_guardar_eventos en el gestor energia
